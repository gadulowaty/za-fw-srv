...
class ExtaTab
{
    ...
    private function colAlign( $iCol, $xAlign )
    {
        if( is_string( $xAlign ) )
        {
            ...
            {
                case "l":
                    return EXTAB_ALIGN_LEFT;
                case "c":
                    return EXTAB_ALIGN_CENTER;
                case "r":
                    return EXTAB_ALIGN_RIGHT;
            }
    ...
    private function strAlign( $sValue, $iWidth, $iAlign )
    {
        switch( $iAlign )
        {
            ...
            case EXTAB_ALIGN_CENTER:
                if( is_array( $sValue ) )
                {
                    return str_pad( $sValue[ 1 ], max( $iWidth + ( strlen( $sValue[ 1 ] ) - strlen( $sValue[ 0 ] ) ), strlen( $sValue[ 0 ] ) ), " ", STR_PAD_BOTH );
                }
            ...
            case EXTAB_ALIGN_RIGHT:
                if( is_array( $sValue ) )
                {
                    return str_pad( $sValue[ 1 ], max( $iWidth + ( strlen( $sValue[ 1 ] ) - strlen( $sValue[ 0 ] ) ), strlen( $sValue[ 0 ] ) ), " ", STR_PAD_LEFT );
                }
